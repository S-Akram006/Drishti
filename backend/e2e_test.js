...
async function runE2ETests() {
  ...
  try {
    ...
    const modalContent = page.locator('text=Blurry image detected').first();
    ...
    const severityHeading = page.locator('text=DR Severity Grade (0 to 4)').first();
    await severityHeading.waitFor({ state: 'visible', timeout: 15000 });
    console.log(green('  ✓ DR Severity Level Gauge visible on disposition card.'));

    const qualityGatePass = page.locator('text=GRADABLE PASS').first();
    await qualityGatePass.waitFor({ state: 'visible', timeout: 15000 });
    ...
    // -------------------------------------------------------------------------
    console.log('\n' + '-'.repeat(80));
    console.log(bold('[SCENARIO 3] District Hospital Telemedicine Queue & Referral Approval'));
    console.log('-'.repeat(80));

    // Submit a referable case using preset "Moderate DR"
    console.log('  -> Clicking "Moderate DR (Referable)" preset button...');
    const modDrPresetBtn = page.locator('button:has-text("Moderate DR")').first();
    await modDrPresetBtn.click();
    await page.waitForTimeout(1000);

    console.log('  -> Submitting referable screening case...');
    await submitBtn.click();

    // Wait for screening completion
    await page.waitForTimeout(4000);

    console.log('  -> Scrolling down to Telemedicine Queue Drawer...');
    const queueDrawerHeader = page.locator('text=District Hospital Telemedicine Queue').first();
    ...
    await queueTable.waitFor({ state: 'visible', timeout: 10000 });
    ...
    console.log(cyan(`  -> Found ${approveCount} pending specialist referral(s) in queue.`));

    if (approveCount > 0) {
      console.log('  -> Clicking "Approve Referral" on the first pending case...');
      await approveButtons.first().click();
      await page.waitForTimeout(2000);
      console.log(green('  ✓ Referral successfully approved by specialist! Status updated.'));
    } else {
    ...
    console.log(yellow(`  Saved error screenshot to: ${path.resolve(__dirname, 'e2e_failure.png')}`));
    throw err;
  } finally {
